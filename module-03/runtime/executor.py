...
def executar_gancho(nome: str, contrato_ganchos: dict, **kwargs):
    ...
    Ganchos são pontos de extensão que permitem executar ações (log, alerta)
    em momentos específicos do ciclo (antes/depois de etapa, antes/depois de ação, em erro).
    """
    ganchos = contrato_ganchos.get("ganchos", {})
    acao = ganchos.get(nome)  # Pega a ação definida para este gancho (ex: "log", "alerta")
    if not acao:
...
_MAPA_TIPOS = {
    "string": str,
    "int": (int,),           # Tupla com um elemento (int)
...
def validar_payload(nome_ferramenta: str, argumentos: dict, contratos: dict) -> list:
    ...
    # Pega o schema de entrada da ferramenta (quais campos são esperados)
    schema_entrada = habilidade.get("entrada", {})
    argumentos = argumentos or {}  # Se veio None, vira dicionário vazio

    # Verifica cada campo obrigatório do schema
    for campo, tipo_esperado in schema_entrada.items():
        # Verifica se o campo está presente nos argumentos
        ...
        # Normaliza o tipo esperado (converte para minúsculo se for string)
...
def validar_saida(nome_ferramenta: str, resultado: dict, contratos: dict) -> list:
    ...
    # Se não tem resultado ou falhou, não precisa validar
    ...
    for campo, tipo_esperado in schema_saida.items():
        ...
        # Verifica se o valor não está vazio/nulo (mínimo de qualidade dos dados)
        if valor is None:
...
def executar(nome_ferramenta: str, argumentos: dict, ferramentas: dict, contratos: dict) -> dict:
    ...
    Retorna um dicionário com sucesso (bool) e os dados ou erro.
    """
    # Verifica se a ferramenta existe no dicionário de ferramentas disponíveis
    ...
    try:
        # Tenta executar a ferramenta com os argumentos fornecidos
...
def avaliar(plano: dict, resultado_acao: dict, contratos: dict = None) -> dict:
    ...
    - qualidade: "completa", "parcial" ou "falha"
    - problemas_saida: lista de problemas encontrados
    """
    ...
    # Avaliação semântica: valida a saída contra o schema da ferramenta
    ...
    # Define qualidade baseada na validação da saída
    if problemas_saida:
    ...
    return {
        "objetivo_alcancado": False,  # Ainda não alcançou o objetivo final
        "motivo": motivo,
        "qualidade": qualidade,
