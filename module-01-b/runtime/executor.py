...
def validar_payload(nome_ferramenta: str, argumentos: dict, contratos: dict) -> list:
    ...
    if not habilidade:
        return [
            f"ferramenta '{nome_ferramenta}' nao encontrada no schema de habilidades"
        ]
    ...
    for campo, tipo_esperado in schema_entrada.items():
        ...
        tipo_normalizado = (
            tipo_esperado.lower() if isinstance(tipo_esperado, str) else "string"
        )
        ...
        if tipos_python and valor is not None:
            ...
            if isinstance(tipos_python, tuple):
                if not isinstance(valor, tipos_python):
                    erros.append(
                        f"campo '{campo}': esperado {tipo_normalizado}, recebido {type(valor).__name__}"
                    )
            # Se é um tipo único
...
def executar(
    nome_ferramenta: str, argumentos: dict, ferramentas: dict, contratos: dict
) -> dict:
    """
    ...
    if nome_ferramenta not in ferramentas:
        return {
            "sucesso": False,
            "erro": f"Ferramenta '{nome_ferramenta}' nao encontrada na caixa de ferramentas",
        }
...
def avaliar(plano: dict, resultado_acao: dict, contratos: dict = None) -> dict:
    ...
    if problemas_saida:
        ...
        # Sucesso completo
        motivo = (
            f"etapa ok - criterio: {criterio}" if criterio else "etapa ok - continuar"
        )
        qualidade = "completa"
