...
import argparse  # Usado para ler e parsear argumentos passados pelo terminal
import io  # Usado para manipular fluxos de entrada/saída (forçar UTF-8 no Windows)
import sys  # Acessa funcionalidades do sistema, como entrada padrão
import json  # Trabalha com dados no formato JSON (trace, análise, etc.)
from pathlib import Path  # Trabalha com caminhos de arquivos/pastas

# Garante UTF-8 no stdout/stderr em qualquer SO (resolve cp1252 no Windows)
if sys.stdout.encoding and sys.stdout.encoding.lower() != "utf-8":
    # Recria o stdout com codificação UTF-8
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8", errors="replace")
    sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding="utf-8", errors="replace")

# Importa funções principais dos módulos do projeto
...
def main():
    ...
    # Subcomando "rastreamento" - exibe rastreamento da última execução
    ...
    # Subcomando "replay" - reexecuta com a mesma entrada
    ...
    if argumentos.comando == "rodar":
        ...
        # Gera relatório markdown da análise
        ...
        # Executa benchmark para uma arquitetura específica
        resultado = rodar_benchmark(
            caminho_agente=argumentos.agente,
            caminho_suite=argumentos.suite,
            arquitetura=getattr(argumentos, "arquitetura", None),
        )
        # Salva resultado individual em arquivo JSON
        ...
        # Executa benchmark para todas as arquiteturas e gera relatório comparativo
