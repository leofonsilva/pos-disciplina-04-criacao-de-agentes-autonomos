...
def _chamar_llm_ferramenta(prompt_sistema: str, prompt_usuario: str, campos_saida: dict) -> tuple:
    ...
    if resposta.usage:
        uso_tokens = {
            "prompt": resposta.usage.prompt_tokens or 0,      # Tokens da mensagem do usuário
...
- Responda em portugues"""
    ...
    def funcao(argumentos):
        ...
        if dados_llm:
            dados_llm["_entrada"] = argumentos  # Adiciona os argumentos recebidos (para rastreamento)
...
def _gerar_valor_fallback(tipo_campo: str, nome_campo: str):
    ...
    if tipo_normalizado == "float":
        return round(random.uniform(0.01, 100.0), 2)  # Número decimal aleatório entre 0.01 e 100
    ...
    if tipo_normalizado == "list":
        return [{"item": f"{nome_campo}_1"}, {"item": f"{nome_campo}_2"}]  # Lista com 2 itens
