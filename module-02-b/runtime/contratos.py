...
def carregar_contratos(caminho_agente: Path, arquitetura: str = None) -> dict:
    ...
    Parâmetros:
        ...
        arquitetura (str, opcional): Nome da arquitetura cognitiva (react, plan_execute, reflect)
    ...
    # Se uma arquitetura foi especificada, tenta carregar contratos específicos dela
    if arquitetura:
        # Caminho para a pasta de arquiteturas (um nível acima do agente)
        ...
        if not pasta_arq.exists():
            ...
            # Tenta carregar planner.md específico da arquitetura
            ...
            # Tenta carregar executor.md específico da arquitetura
            ...
            # NOVO: tenta carregar critic.md específico da arquitetura (usado em arquiteturas como "reflect")
...
def criar_estado(contratos: dict, texto_entrada: str, modo: str = None, evento: str = None, arquitetura: str = None) -> dict:
    ...
    return {
        ...
        "arquitetura": arquitetura or "padrao",  # NOVO: arquitetura cognitiva (padrao se não especificada)
        ...
        "sem_progresso": regras.get("limites", {}).get("sem_progresso", 3),  # Detecção de estagnação
        ...
        "acoes_sensiveis": regras.get("acoes_sensiveis", []),  # Ações que precisam confirmação humana
