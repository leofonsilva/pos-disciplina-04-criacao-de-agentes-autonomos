...
from pathlib import Path  # Trabalha com caminhos de arquivos/pastas de forma mais simples
...
from ciclo import rodar, replay, exibir_rastreamento  # Funções que controlam o ciclo do agente
...
def _resumir_trace(dados: dict) -> str:
    ...
    linhas.append(f"TIPO: {dados.get('tipo_agente', '?')}")  # Tipo do agente (ex: monitor, analyzer...)
    linhas.append(f"TEMPO_TOTAL: {dados.get('tempo_total_segundos', 0)}s")  # Duração total em segundos
    linhas.append(f"TOKENS: {json.dumps(dados.get('tokens_consumidos', {}))}")  # Tokens usados pela LLM
    ...
    for etapa in dados.get("etapas", []):  # Percorre cada etapa do pipeline
        ...
        sucesso = resultado.get("sucesso", False) if resultado else None  # Se a ação foi bem sucedida
        avaliacao = etapa.get("avaliacao", {})  # Avaliação da qualidade da ação
        qualidade = avaliacao.get("qualidade", "")  # Qualidade da ação (ex: "bom", "ruim")
        objetivo = avaliacao.get("objetivo_alcancado", False)  # Se o objetivo foi alcançado
...
def main():
    ...
    parser_rodar.add_argument("--arquitetura", required=False, help="Arquitetura cognitiva (react, plan_execute, reflect)")  # NOVO
    ...
    if argumentos.comando == "rodar":
        rodar(
            ...
            arquitetura=getattr(argumentos, "arquitetura", None),  # NOVO: passa arquitetura para a função
        )
