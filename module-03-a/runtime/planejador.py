...
def perceber(estado: dict) -> str:
    ...
    ferramentas usadas, progresso e avisos de estagnação.
    """
    partes = [f"Alerta: {estado['entrada']}"]

    # Adiciona o modo de operação do agente
    ...
    # Aviso de estagnação (repetição de ferramenta)
...
def construir_prompt_sistema(contratos: dict) -> str:
    ...
    # Constroi bloco com descrição de todas as ferramentas disponíveis
    ...
    # Regras do planejador (como deve tomar decisões)
    ...
    # Formato de saída (pode ser customizado por arquitetura)
    ...
    if isinstance(formato_saida, dict) and formato_saida:
        ...
        # Formato padrão (backward compatible)
        bloco_formato = """{
  ...
  "pergunta": "pergunta para o usuario (obrigatorio se PERGUNTAR_USUARIO)"
    ...
    # Políticas do agente (restrições de segurança/comportamento)
...
def chamar_llm(percepcao: str, contratos: dict, historico: list = None) -> tuple:
    ...
    Parâmetros:
    - percepcao: contexto atual montado pela função perceber()
    - contratos: definições do agente (ferramentas, regras, etc.)
    - historico: lista de etapas já executadas

    Retorna (plano, uso_tokens) onde plano é um dicionário com a decisão
    e uso_tokens contém {prompt, completion, total}.
    """
    chave_api = os.environ.get("OPENAI_API_KEY")

    # Se não tem API key, usa o planejador mock (fallback)
    if not chave_api:
    ...
    try:
        ...
        # Se falhar, retorna um plano de finalização com erro
...
def planejador_mock(percepcao: str, contratos: dict, historico: list = None) -> dict:
    ...
    Usado quando não há API key ou para testes. Segue uma lógica simples:
    1. Se é modo plan_execute e não há histórico, gera plano completo
    2. Se é modo interactive e não há histórico, pergunta ao usuário
    3. Senão, chama a próxima ferramenta não usada
    ...
    # Descobre qual a próxima ferramenta não usada
