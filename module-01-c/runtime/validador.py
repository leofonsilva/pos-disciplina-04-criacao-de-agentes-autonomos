...
def validar(caminho_agente: str) -> bool:
    ...
    habilidades = carregar_yaml_do_md(
        caminho / "skills.md"
    )  # Lista de habilidades/ferramentas
    toolbox = carregar_yaml_do_md(
        pasta_contratos / "toolbox.md"
    ...
    nomes_habilidades = {
        h["nome"] for h in habilidades.get("habilidades", []) if "nome" in h
    }
    ...
    for nome in nomes_toolbox - nomes_habilidades:
        erros.append(
            f"  [ERRO] ferramenta '{nome}' esta no toolbox.md mas nao em skills.md"
        )
    ...
    for nome in nomes_habilidades - nomes_toolbox:
        avisos.append(
            f"  [AVISO] ferramenta '{nome}' esta em skills.md mas nao no toolbox.md"
        )
    ...
    for nome in regras.get("ferramentas_obrigatorias", []):
        if nome not in nomes_habilidades:
            erros.append(
                f"  [ERRO] ferramenta obrigatoria '{nome}' nao existe em skills.md"
            )
    ...
    if isinstance(chamadas, dict):
        for nome in chamadas:
            ...
            if nome != "total" and nome not in nomes_habilidades:
                avisos.append(
                    f"  [AVISO] limite definido para '{nome}' que nao existe em skills.md"
                )
    ...
    if tipo and tipo not in tipos_validos:
        erros.append(
            f"  [ERRO] tipo '{tipo}' invalido. Valores: {', '.join(tipos_validos)}"
        )
