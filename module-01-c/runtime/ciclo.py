...
import json  # Para trabalhar com arquivos JSON (trace, configurações, etc.)
...
from ferramentas import (
    construir_ferramentas_dos_contratos,
...
def exibir_kpis(estado: dict, tel, inicio: float, contratos: dict):
    ...
    max_chamadas = estado[
        "max_chamadas_ferramenta"
    ]  # Total máximo de chamadas a ferramentas
    max_tokens = estado.get(
        "max_tokens", 50000
    )  # Limite máximo de tokens (padrão 50000)
    limite_tempo = estado.get(
        "limite_tempo_segundos", 120
    )  # Limite de tempo em segundos (padrão 120)

    etapa = estado["etapa"]  # Etapa atual (começa em 1)
    chamadas = estado[
        "chamadas_ferramenta"
    ]  # Número de chamadas a ferramentas já feitas
    tokens_total = estado["tokens_consumidos"]["total"]  # Total de tokens consumidos
    tempo_decorrido = round(
        time.time() - inicio, 1
    )  # Tempo decorrido (arredondado para 1 decimal)
    ...
    barra = "\u2593" * blocos_cheios + "\u2591" * (
        10 - blocos_cheios
    )  # Caractere bloco cheio + vazio
    pct_str = f"{pct_tokens * 100:.1f}%"  # Porcentagem formatada

    # Informações sobre ferramentas: já chamadas, obrigatórias, pendentes
    habilidades = contratos.get("habilidades", {}).get(
        "habilidades", []
    )  # Lista de habilidades/ferramentas
    obrigatorias = set(
        contratos.get("regras", {}).get("ferramentas_obrigatorias", [])
    )  # Ferramentas obrigatórias
    nomes_ferramentas = [
        h["nome"] for h in habilidades
    ]  # Extrai só os nomes das ferramentas
    ...
    for nome in nomes_ferramentas:
        if nome in estado["chamadas_por_ferramenta"]:
            ...
            partes_ferramentas.append(
                f"!{nome}"
            )  # Ponto de exclamação - obrigatória pendente
        else:
            partes_ferramentas.append(
                f"\u25cb{nome}"
            )  # Círculo vazio - não chamada ainda
    ...
    lat = tel.kpis_etapa(
        etapa
    )  # Obtém latências por fase (perceber, planejar, agir, avaliar)
    partes_lat = [
        f"{fase}={int(ms)}ms" for fase, ms in lat.items()
    ]  # Formata cada fase
    ...
    print(
        f"  \│ Progresso:  {etapa}/{max_etapas} etapas    {chamadas}/{max_chamadas} chamadas    {tempo_decorrido}s/{limite_tempo}s"
    )
    print(f"  \│ Tokens:     {tokens_total}/{max_tokens} ({pct_str})  {barra}")
    print(f"  \│ Ferramentas: {texto_ferramentas}")
    print(
        f"  \│ Qualidade:  {ok}/{ok + parcial + falha} ok   {parcial} parcial   {falha} falha"
    )
...
def pedir_confirmacao_humana(nome_ferramenta: str) -> bool:
    ...
    try:
        resposta = (
            input(f"  Autorizar execucao de '{nome_ferramenta}'? (s/n): ")
            .strip()
            .lower()
        )
...
def validar_resposta_llm(plano: dict, ferramentas_disponiveis: set) -> list:
    ...
    if not proxima_acao:
        ...
        problemas.append(
            f"proxima_acao '{proxima_acao}' invalida (validas: {', '.join(_ACOES_VALIDAS)})"
        )
    ...
    if proxima_acao == "CHAMAR_FERRAMENTA":
        ...
        if not nome:
            ...
            problemas.append(
                f"ferramenta '{nome}' nao existe (disponiveis: {', '.join(ferramentas_disponiveis)})"
            )
        ...
        if args is not None and not isinstance(args, dict):
            problemas.append(
                f"argumentos_ferramenta deve ser dict, recebido {type(args).__name__}"
            )
...
def rodar(
    ...
    Parâmetros:
        caminho_agente (str): Caminho para a pasta do agente (contém contracts.json, etc.)
    ...
    """
    # Converte caminho para Path object (resolve caminho absoluto)
    caminho_agente = Path(caminho_agente).resolve()

    # Carrega contratos do agente (ferramentas, regras, limites, etc.)
    contratos = carregar_contratos(caminho_agente)

    # Cria estado inicial do agente (etapa=0, histórico vazio, etc.)
    ...
    while not estado["concluido"] and estado["etapa"] < estado["max_etapas"]:
        ...
        if verificar_tempo(estado, inicio):
            print(
                f"  [regras] limite de tempo excedido ({estado['limite_tempo_segundos']}s)"
            )
            tel.registrar(
                "limite_tempo_excedido", {"segundos": estado["limite_tempo_segundos"]}
            )
        ...
        if verificar_limite_tokens(estado):
            print(
                f"  [regras] limite de tokens excedido ({estado['tokens_consumidos']['total']}/{estado['max_tokens']})"
            )
            tel.registrar("limite_tokens_excedido", estado["tokens_consumidos"])
            estado["concluido"] = True
            estado["resultado"] = (
                f"encerrado por limite de tokens ({estado['tokens_consumidos']['total']})"
            )
            break
        ...
        print(
            f"  [planejar] proxima_acao={plano.get('proxima_acao')} ferramenta={plano.get('nome_ferramenta')} ({marcador_planejar['duracao_ms']}ms, tokens={uso_tokens_plano['total']})"
        )
        ...
        if problemas_llm:
            ...
            print(
                f"  [circuit_breaker] resposta da LLM rejeitada: {'; '.join(problemas_llm)}"
            )
            ...
            if (
                ...
                print(
                    f"  [circuit_breaker] auto-correcao: proxima_acao -> CHAMAR_FERRAMENTA, ferramenta={nome_no_plano}"
                )
            ...
            # Fallback: ferramenta não existe, redireciona para próxima não usada
                ...
                if ferramenta_fallback:
                    habilidade_fb = next(
                        h for h in habilidades if h["nome"] == ferramenta_fallback
                    )
                    # Cria um novo plano com a ferramenta fallback
                    plano = {
                        "proxima_acao": "CHAMAR_FERRAMENTA",
                        "nome_ferramenta": ferramenta_fallback,
                        "argumentos_ferramenta": montar_argumentos_mock(
                            habilidade_fb, estado["historico"]
                        ),
                        "criterio_sucesso": f"fallback apos circuit breaker: {ferramenta_fallback}",
                    }
                    print(
                        f"  [circuit_breaker] redirecionando para fallback: {ferramenta_fallback}"
                    )
                else:
                    # Sem fallback disponível, encerra
                    estado["concluido"] = True
                    estado["resultado"] = (
                        f"encerrado por circuit breaker: {'; '.join(problemas_llm)}"
                    )
                    break
            else:
            ...
            estado["historico"].append(
                {
                    ...
                    "resultado_acao": {
                        "sucesso": True,
                        "dados": {"resposta_usuario": resposta_usuario},
                    },
                    "avaliacao": {
                        "objetivo_alcancado": False,
                        "motivo": "aguardando resposta do usuario",
                    },
                }
            )
        ...
        if plano.get("proxima_acao") == "FINALIZAR":
            ...
            obrigatorias = contratos.get("regras", {}).get(
                "ferramentas_obrigatorias", []
            )
            # Verifica quais ainda não foram chamadas
            faltantes = [
                nome_obrigatoria
                for nome_obrigatoria in obrigatorias
                if nome_obrigatoria not in estado["chamadas_por_ferramenta"]
            ]
            if faltantes:
                print(
                    f"  [regras] ferramentas obrigatorias pendentes: {', '.join(faltantes)}"
                )
                ...
                plano = {
                    ...
                    "argumentos_ferramenta": montar_argumentos_mock(
                        habilidade_faltante, estado["historico"]
                    ),
        ...
        # --- FASE 3: AGIR ---
        resultado_acao = None
        if plano.get("proxima_acao") == "CHAMAR_FERRAMENTA" and plano.get(
            "nome_ferramenta"
        ):
            ...
            if estado["chamadas_ferramenta"] >= estado["max_chamadas_ferramenta"]:
                print(
                    f"  [regras] limite total de chamadas de ferramenta atingido ({estado['max_chamadas_ferramenta']})"
                )
                estado["concluido"] = True
                estado["resultado"] = (
                    "encerrado por limite total de chamadas de ferramenta"
                )
                break

            # Verifica limite específico desta ferramenta
            chamadas_desta_ferramenta = estado["chamadas_por_ferramenta"].get(
                nome_ferramenta, 0
            )
            limite_desta_ferramenta = estado["limites_por_ferramenta"].get(
                nome_ferramenta
            )
            if (
                limite_desta_ferramenta
                and chamadas_desta_ferramenta >= limite_desta_ferramenta
            ):
                print(
                    f"  [regras] limite de {nome_ferramenta} atingido ({limite_desta_ferramenta})"
                )
                estado["concluido"] = True
            ...
            if verificar_sem_progresso(estado, nome_ferramenta):
                print(
                    f"  [regras] sem progresso detectado - {estado['sem_progresso']} chamadas consecutivas a '{nome_ferramenta}'"
                )
                estado["concluido"] = True
                estado["resultado"] = (
                    f"encerrado por estagnacao (ferramenta repetida: {nome_ferramenta})"
                )
                break
            ...
            if nome_ferramenta in estado.get("acoes_sensiveis", []):
                ...
                if not pedir_confirmacao_humana(nome_ferramenta):
                    ...
                    estado["concluido"] = True
                    estado["resultado"] = (
                        f"encerrado por negacao humana ({nome_ferramenta})"
                    )
                    break
            ...
            erros_payload = validar_payload(
                nome_ferramenta, plano.get("argumentos_ferramenta"), contratos
            )
            ...
            executar_gancho(
                "antes_da_acao", contrato_ganchos, ferramenta=nome_ferramenta
            )
            resultado_acao = executar(
                nome_ferramenta,
                plano.get("argumentos_ferramenta"),
                ferramentas,
                contratos,
            )
            ...
            estado["chamadas_por_ferramenta"][nome_ferramenta] = (
                chamadas_desta_ferramenta + 1
            )
            ...
            if not sucesso:
                executar_gancho(
                    "em_erro", contrato_ganchos, erro=resultado_acao.get("erro", "")
                )

            print(
                f"  [agir] resultado={json.dumps(resultado_acao, ensure_ascii=False)[:100]} ({marcador_agir['duracao_ms']}ms)"
            )
        ...
        print(
            f"  [avaliar] objetivo_alcancado={avaliacao['objetivo_alcancado']} - {avaliacao['motivo']} ({marcador_avaliar['duracao_ms']}ms)"
        )
    ...
    print(
        f"  Tokens consumidos: {estado['tokens_consumidos']['total']} (prompt={estado['tokens_consumidos']['prompt']}, completion={estado['tokens_consumidos']['completion']})"
    )
    ...
    for nome_fase, dados_fase in perf["fases"].items():
        print(
            f"  {nome_fase}: media={dados_fase['media_ms']}ms max={dados_fase['max_ms']}ms total={dados_fase['total_ms']}ms ({dados_fase['contagem']}x)"
        )
    ...
    # Salva rastreamento em arquivo JSON
    dados_rastreamento = {
    ...
    caminho_rastreamento = (
        Path(saida) if saida else Path(__file__).parent / "trace.json"
    )
...
def exibir_rastreamento():
    ...
    for registro in historico:
        ...
        print(f"Etapa {etapa}")
        print(
            f"  plano     : {plano.get('proxima_acao')} -> {plano.get('nome_ferramenta', '-')}"
        )
        print(f"  criterio  : {plano.get('criterio_sucesso', '-')}")
        if resultado:
            situacao = "ok" if resultado.get("sucesso") else "falha"
            print(
                f"  acao      : {situacao} - {json.dumps(resultado.get('dados', resultado.get('erro', '')), ensure_ascii=False)[:80]}"
            )
        qualidade = avaliacao.get("qualidade", "")
        print(
            f"  avaliacao : objetivo_alcancado={avaliacao.get('objetivo_alcancado')}{f' qualidade={qualidade}' if qualidade else ''}"
        )
        print()
    ...
    if metadados.get("performance_data"):
        ...
        for nome_fase, dados_fase in perf.get("fases", {}).items():
            print(
                f"  {nome_fase}: media={dados_fase['media_ms']}ms max={dados_fase['max_ms']}ms"
            )
        print()
