...
def construir_prompt_sistema(contratos: dict) -> str:
    ...
    # NOVO: formato de saida — lido do contrato (permite que arquiteturas mudem o formato)
...
def chamar_llm(percepcao: str, contratos: dict, historico: list = None) -> tuple:
    ...
    Retorna:
        ...
        - uso_tokens (dict): Quantidade de tokens consumidos (com campo "_modo" adicional)
    """
    ...
    if not chave_api:
        ...
        tokens_mock["_modo"] = "mock"  # NOVO: indica que usou mock em vez de LLM
    ...
    resposta = cliente.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": construir_prompt_sistema(contratos)},  # Instruções do sistema
    ...
    try:
        ...
        uso_tokens["_modo"] = "llm"  # NOVO: indica que usou LLM real
...
def planejador_mock(percepcao: str, contratos: dict, historico: list = None) -> dict:
    ...
    # NOVO: detecta se a arquitetura produz raciocinio (campo presente no formato_saida)
    ...
    if tipo_agente == "interactive" and not historico:
        ...
        # NOVO: adiciona raciocínio se a arquitetura suportar
