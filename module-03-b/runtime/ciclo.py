...
import json  # Para salvar trace em JSON
import sys  # Para acesso a funções do sistema (não usado diretamente)
import time  # Para medir tempo de execução
from pathlib import Path  # Para manipular caminhos de arquivos

from contratos import carregar_contratos, criar_estado  # Carrega contratos e cria estado inicial
from executor import avaliar, executar, executar_gancho, validar_payload  # Funções de execução
from ferramentas import construir_ferramentas_dos_contratos, montar_argumentos_mock  # Constrói ferramentas
from planejador import _TOKENS_ZERO, chamar_llm, perceber  # Planejamento
from telemetria import Telemetria  # Coleta de métricas
...
def exibir_kpis(estado: dict, tel, inicio: float, contratos: dict):
    ...
    # barra visual de tokens (▓ = usado, ░ = disponível)
    ...
    # ferramentas: ✓ já usada, ! obrigatória não usada, ○ disponível
...
def _executar_critica(estado: dict, contratos: dict, contrato_critico: dict) -> dict:
    ...
    if chave_api:
        try:
            ...
            # montar contexto de critica (resumo do histórico)
            historico_resumo = []
            ...
            # formata os criterios como texto
            criterios_texto = "\n".join(
...
{{
  ...
  "sugestoes": ["sugestao 1", "sugestao 2"]
    ...
    # fallback mock: primeira reflexao rejeita, segunda aprova
...
def rodar(caminho_agente: str, texto_entrada: str, modo: str = None, evento: str = None, saida: str = None, arquitetura: str = None) -> dict:
    ...
    while not estado["concluido"] and estado["etapa"] < estado["max_etapas"]:
        ...
        # verificar limites (tempo e tokens)
        ...
        # Usa plan_execute_total como sentinela para distinguir "plano ja existe"
        # de "plano ainda nao foi gerado na primeira etapa".
        ...
        if modo_execucao == "plan_execute" and plano_ja_gerado:
            ...
            if plano_armazenado:
                ...
                # Plano concluído, sem passos restantes
                plano = {
            ...
            # Chamar LLM para planejar (modo normal ou primeira etapa do plan_execute)
            marcador_planejar = tel.iniciar_fase("planejar", estado["etapa"])
            ...
            if modo_execucao == "plan_execute" and plano.get("plano_completo"):
                ...
                if len(passos) > 1:
                    ...
                    estado["plano_completo"] = []  # Só tinha um passo
        ...
        if problemas_llm:
            ...
            # auto-correcao: ação inválida mas nome da ferramenta é válido
            ...
            # fallback: ferramenta não existe, redirecionar para próxima não usada
        ...
        # --- FASE: REFLEXAO (Reflection) - autoavaliação antes de finalizar ---
        ...
        if plano.get("proxima_acao") == "FINALIZAR" and contrato_critico:
            ...
            if reflexoes_feitas < max_reflexoes:
                ...
                if aprovado or nota >= limiar:
                    ...
                    # redirecionar para a ferramenta sugerida
        ...
        if plano.get("proxima_acao") == "CHAMAR_FERRAMENTA" and plano.get("nome_ferramenta"):
            ...
            # verifica estagnação (mesma ferramenta repetida)
            ...
            if erros_payload:
                ...
                # não bloqueia — apenas registra (graceful degradation)
