...
import time  # Para medir tempo total de execução
from pathlib import Path  # Para manipular caminhos de arquivos

import yaml  # Para ler arquivos YAML (eval suite)

from ciclo import rodar  # Importa a função que executa o agente
...
def _extrair_metricas_trace(trace: dict, caso: dict) -> dict:
    ...
    etapas = trace.get("historico", [])  # Lista de etapas executadas
    ...
    # Conjunto para armazenar ferramentas chamadas (sem duplicatas)
    ferramentas_chamadas = set()
    
    # Dicionário para contar qualidade das etapas
    ...
    # Calcula cobertura de ferramentas esperadas
...
def rodar_benchmark(caminho_agente: str, caminho_suite: str, arquitetura: str = None) -> dict:
    ...
    # Itera sobre cada cenário do dataset
    ...
    agregado = {
        ...
        # Taxa de sucesso: (completa + parcial) / total de execuções * 100
...
def gerar_relatorio_comparativo(resultados: list, caminho_saida: str):
    ...
    md.append("|" + "---|" * (len(resultados) + 1))  # Linha separadora
    ...
    for nome, chave, sufixo in metricas_exibir:
        ...
        else:  # Para as demais (taxa_conclusao, cobertura), maior é melhor
            melhor = max(nums)
