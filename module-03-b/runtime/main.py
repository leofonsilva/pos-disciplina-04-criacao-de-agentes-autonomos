...
Uso:
  ...
  python main.py tool-eval --agente ../monitor-agent --suite ../evals/suites/tool-selection.yaml
  python main.py tool-eval-comparar --agente ../monitor-agent --suite ../evals/suites/tool-selection.yaml
"""

import argparse  # Para processar argumentos da linha de comando
import io  # Para manipular fluxos de entrada/saída
import sys  # Para acessar o sistema e configurar encoding
import json  # Para ler e escrever arquivos JSON
from pathlib import Path  # Para manipular caminhos de arquivos

# Garante UTF-8 no stdout/stderr em qualquer SO (resolve problemas de encoding no Windows)
...
from tool_eval import rodar_tool_eval, gerar_relatorio_tool_eval  # NOVO: tool selection evaluation
...
def _resumir_trace(dados: dict) -> str:
    ...
    Converte o trace completo (que pode ser grande) em um texto resumido
    que será passado como entrada para o agente analyzer.
    """
    linhas = []
    ...
    # Processa cada etapa do histórico
    for etapa in dados.get("etapas", []):
        ...
        # Adiciona linha resumida da etapa
        linhas.append(
    ...
    # Health metrics (métricas de saúde do agente)
...
def _gerar_relatorio_md(dados_trace: dict, dados_analise: dict) -> str:
    ...
    Parâmetros:
    - dados_trace: trace original da execução do agente
    - dados_analise: resultado da análise feita pelo trace-analyzer

    Retorna uma string em formato Markdown pronta para ser salva como relatório.
    """
    ...
    # Detalhamento por fase (performance)
...
def main():
    ...
    subparsers = parser.add_subparsers(dest="comando")  # Subcomandos
    ...
    # Subcomando: rastreamento (exibe trace da última execução)
    subparsers.add_parser("rastreamento", help="Exibe o rastreamento da ultima execucao")

    # Subcomando: analisar (analisa trace com agente analyzer)
    ...
    # Subcomando: replay (reexecuta com mesma entrada)
    ...
    # Subcomando: benchmark (roda benchmark de uma arquitetura)
    ...
    # Subcomando: comparar (roda benchmark das arquiteturas e gera relatório)
    ...
    # NOVO: Subcomando tool-eval (avalia precisão de tool selection)
    ...
    # NOVO: Subcomando tool-eval-comparar (compara tool selection entre arquiteturas)
    ...
    if argumentos.comando == "rodar":
        ...
        # Define o caminho do trace (padrão ou fornecido)
        ...
        # Roda o benchmark para uma arquitetura específica
        ...
        # Roda benchmark para todas as arquiteturas e gera relatório comparativo
    ...
    # NOVO: tool-eval - avalia tool selection do agente
    ...
    # NOVO: tool-eval-comparar - compara tool selection entre arquiteturas
        ...
        # Se nenhum comando válido foi fornecido, mostra ajuda
        parser.print_help()
